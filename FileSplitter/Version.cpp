...
class CVersion {
  ...
  VOID init( LPTSTR filename )
  {
    ...
    UINT len;
    if( GetFileVersionInfo( filename, NULL, size, pData ) ){
...
public:
  ...
  CVersion( CString Filename )
  {
    init( (LPTSTR)(LPCSTR)Filename );
  }
  ...
  CString GetComments()
  {
    ...
    CHAR *p;
    UINT len;
    VerQueryValue( pData, (LPTSTR)(LPCSTR)SubBlock, (LPVOID *)&p, &len );
